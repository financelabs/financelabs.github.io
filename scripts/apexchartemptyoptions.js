(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.labs = {}));
	...
	function prepareApexChartByPostType({
	  showAs = "",
	  series = [],
	  categories = [],
	  seriesWithNames = null,
	  chartTitle = "",
	  xaxisTitle = "",
	  yaxisTitle = "",
	  options
	}) {
	  if (showAs === "apexbarchartsgrouped" || showAs === "arraycategoriesbarapexchart") {
	    // let series = [
	    ...
	    // ];
	    // let categories = ['1кв. 2022г.', '2кв. 2022г.', '3кв. 2022г.', '4кв. 2022г.'];
	    return {
	      xaxis: {
	  ...
	  if (showAs === "datalinebarapexchart" || showAs === "datetimebarapexchart") {
	    return {
	      chart: {
	        type: 'bar',
	        toolbar: {
	      ...
	      // columnWidth: '55%', endingShape: 'rounded'
	      dataLabels: {
	        enabled: false
	      },
	      stroke: {
	  ...
	  if (showAs === "scatterquotesapexchart" || showAs === "scatterapexchart") {
	    let scatterSeries = (() => {
	      if (!!seriesWithNames) {
	  ...
	  if (showAs === "datalinequotesapexchart" || showAs === "datetimeareanocategoriesapexchart") {
	    // series = [
	    //   { name: "dataline1", data: [['Dec 23 2017', 100], ['Dec 23 2020', 120]] },
	    //   { name: "dataline2", data: [['Dec 23 2017', 150], ['Dec 23 2020', 80]] }
	    // ]
	    return {
	      chart: {
	        type: "area",
	...
	function xy_beta_random(number) {
	  return [...new Array(number)].map(() => [Math.round(Math.random() * 300 - 140) / 10, Math.round(Math.round(Math.random() * 300 - 140)) / 10]);
	}
	...
	function prepareScatterChartAbstractBetaCases({
	  type = "betamorethanone",
	  myComponentFeatures = {
	    scatteroptions: {
	      xaxis: {
	        title: {
	          text: "Index month return, %"
	        }
	      },
	      yaxis: {
	        title: {
	          text: "Asset month return, %"
	        }
	      }
	    }
	  }
	  ...
	  let xy_data = (() => {
	    if (type === "betamorethanone") {
	      return xy_beta_more_than_one(49);
	    }

	    if (type === "betabelowzero") {
	  ...
	  xy_data.map((item, index) => {
	    indexData[index + 1] = [dateInArray(index + 1), Math.round(indexData[index][1] * (1 + item[0] / 100))];
	    assetData[index + 1] = [dateInArray(index + 1), Math.round(assetData[index][1] * (1 + item[1] / 100))];
	    return null;
	  });
	  let areaseries = [{
	    name: "Index",
	...
	const features = {
	  options: {
	    xaxis: {
	      title: {
	        text: "Доходность индекса за месяц, %"
	      }
	    },
	    yaxis: {
	      title: {
	...
	function apexchart(props) {
	  //console.log(props);
	  if (props.type.includes("beta")) {
	    // let charts = prepareScatterChartAbstractBetaCases({ type: props.type });
	    let charts = prepareScatterChartAbstractBetaCases({
	      type: props.type
	    }); // console.log(charts);

	    let labs = document.getElementById("labs");
	    labs.style.padding = "2rem";
	    let scatterchart = document.createElement("section");
	    labs.appendChild(scatterchart);
	    let chartScatter = new ApexCharts(scatterchart, prepareApexChartByPostType(lodash.exports.merge({}, charts[0], features)));
	    chartScatter.render();
	    let areachart = document.createElement("section");
	    labs.appendChild(areachart);
	    let chartArea = new ApexCharts(areachart, prepareApexChartByPostType(charts[1]));
	    chartArea.render();
	    return null;
	  }
	  ...
	  let chart = new ApexCharts(document.getElementById("labs"), options);
	  return chart.render();
	}
...
//# sourceMappingURL=apexchartemptyoptions.js.map
